...
We will use the weather dataset for 17 stations between 2020 to 2024 from [Meteorological Service Singapore (MSS)](http://www.weather.gov.sg/climate-historical-daily/). After cleaning and preparing the data, we narrowed it down to 11 stations. The steps taken during the data preparation process are explained in detail.
...
### 4.1 Compare Temperature Range (Max - Min) Across Stations by Region
...
weather %>%
  ...
  geom_line() +
  labs(title = "Monthly Temperature Patterns by Station", x = "Month", y = "Avg Temperature (°C)")
